      ...
      for(lfc in lfc_cutoffs){
        for(fdr in pval_adj_cutoffs){
          ...
          GenesSampleWise = sample_wise_DEGs$GENE[sample_wise_DEGs$Cell_type==ct &
                                                    ...
                                                    rowSums(abs(sample_wise_DEGs[,grep(fc_type,colnames(sample_wise_DEGs))])>lfc)>=nTotalSamples] # select the genes where the absolute expression FC of each replicate in the two comparion groups all pass the lfc threshold. (i.e. the avglfc of all 6 samples have to pass the lfc cutoff in order to count the gene as sample-wise DEGs given certain FDR cutoff and certain cell type) 
...
subclusterAnalysis <- function(seuratObject, 
                               celltype, 
  ...
  for(ct in unique(markers$cluster)){
    marker_genes[[ct]] = markers$gene[markers$cluster==ct][1:nMarkers] 
    # marker_genes[[ct+1]] = markers$gene[markers$cluster==ct][1:nMarkers] 
    # YZ: index in list start from 1 in R?? therefore, should be marker_genes[[ct+1]] not marker_genes[[ct]]??
    all_marker = append(all_marker, markers$gene[markers$cluster==ct][1:nMarkers])
  }
  dup_genes = all_marker[duplicated(all_marker)]
  ...
  if(length(dup_genes)>0){
    ...
    print(dup_genes)
    for(dgene in dup_genes){ # check every duplicated gene
      for(ct in 1:length(marker_genes)){ # check every duplicated gene in every 'cluter' of marker_genes
        if(dgene %in% marker_genes[[ct]]){ # if the duplicated gene is found in a cluster of marker gene: 
          # can "keep" this marker gene
          # but need to take out of other clusters
          for(clust in (ct+1):length(marker_genes)){ # check every following cluster in the marker_genes list: 
            if(dgene %in% marker_genes[[clust]]){ # if the duplicated gene is found in that following cluster
              marker_genes[[clust]] = marker_genes[[clust]][marker_genes[[clust]]!=dgene] # remove the duplicated genes in that following cluster of marker_genes, only keep the genes that don't have duplicated in that cluster
              for(gene in markers$gene[markers$cluster==names(marker_genes)[clust]]){ # select genes from that folloiwng cluster in the 'markers' df. 
                if(!(gene %in% all_marker)){ # if the gene is not showing up in all_marker df: 
                  marker_genes[[clust]] = append(marker_genes[[clust]], gene) # replace with next marker gene
                  # update all_marker
                  all_marker = c()
                  # for(ct in unique(seuratObject@active.ident)){ ## why 'unique(seuratObject@active.ident)'??? 
  ...
  data <- seuratObject@assays$RNA@scale.data[all_marker,] # get the scaled data of these marker genes for all cells, put them into object 'data'
  
  ordered_data = data.frame(stringsAsFactors = FALSE)
  first = FALSE
  for(ct in levels(seuratObject@meta.data[,resolution])){ # total of 15 levels when res = 0.5
    cells = rownames(meta.data)[meta.data[,"Cluster"]==ct] # e.g., when ct = 0, get all cell (names) that have cluster = 0 in the meta.data, put them into object 'cells'.  
    if(!first){
      ordered_data <- rbind(ordered_data, data[,cells])  # e.g., attach the scaled df (corresponding to the marker genes) of only cells in cluster 0, attachit to ordered_data (by row)
      first = TRUE
    }
    else{
      ordered_data <- cbind(ordered_data, data[,cells]) # e.g., when cluster = 1, bind the cells-marker genes df to the ordered_data by column. Note: the marker genes are the same, therefore we combine the df by columns. 
    }
  }
  ...
  png(file=paste0("./Subclustering/",celltype,"/MinMax5_MarkerGenes_",resolution,"_2.png"), 
  ...
  for(meta in proportionMetaData){
    ...
    pdf(paste0("Macrophage_subclusters_prop",meta,"_SampleRemoveDoub.pdf"))
    # print(propPlot[[1]])
    print(propPlot)
    dev.off()
    combined_df[[meta]] <- as.data.frame(propPlot[[1]])
    # combined_df[[meta]] <- as.data.frame(propPlot)
  }
  ...
  if(combinePropPlots){
    ...
    combined <- ggplot(toPlot, aes(x = as.numeric(interaction(Meta,RNA_snn_res.0.5)), y = Percentage, fill = Variable)) + 
      ...
      facet_grid(.~RNA_snn_res.0.5, scales = "free", space = "free") +
      xlab("Cluster") +
      theme_bw() +
      theme(panel.border = element_blank(), 
  ...
  pathwayEnrichment(modules_list = modfile_pathway[,c("MODULE","GENE")], 
                    resources_path = pathway_resource, 
...
processSeurat <- function(seuratObject, 
  ...
  DefaultAssay(seuratObject) <- "RNA"
  # DefaultAssay(seuratObject) <- "decontXcounts" # only for the purpose of decontX results 
  seuratObject <- NormalizeData(seuratObject)
  seuratObject <- FindVariableFeatures(seuratObject)
  ...
  if(is.null(facet_by)){
    D <- ggplot(selected, aes_string(x=vars[1], y=vars[2], size="neglog10fdr", color=logFCtype)) + 
      ...
      theme(panel.border = element_blank(), #panel.grid.major = element_blank(),
            ...
            legend.text = element_text(size=10),
            legend.position = "bottom") +
      labs(size=expression(-log[10]~p["Adj"]), color=expression(log2FC)) + # YZ 061422: changed logFC to log2FC
      scale_color_gradient2(low = colors[1],  mid=colors[2], high = colors[3]) +
      #breaks = c(-.5, 0, .5), labels = c(-.5, 0, .5)) +
      scale_size(range = c(3, 12), breaks = c(1.3,25,50,75,100)) +
      scale_shape_manual(values=shapes) +
  ...
  for(module in unique(DEG_df$MODULE)){
    ...
    for(z in 1:length(list_of_pathway_databases)){
      ...
      for(k in 1:Module2_len){
        ...
        data_matrix_for_enrichment[List_initial, 5] <- 25000
        data_matrix_for_enrichment[List_initial,6] <- Unique_module2[k]
...
summarizeTopDegs <- function(DEG_df, 
  ...
  # ensembl = useEnsembl(biomart='ensembl', dataset=ensembl_dataset, mirror = "useast") # YZ test
      ...
      if(sum(subset$GENE==DEG)>0){
        celltype_group = append(celltype_group,
                                concatenate(gsub(paste0(comp,"_"),"",subset[,MODULE][subset$GENE==DEG]),
                                            mysep = ", "))
        logFCs = append(logFCs, concatenate(gsub(paste0(comp,"_"),"",subset$avg_log2FC[subset$GENE==DEG]), 
                                            mysep = ", "))
      ...
      if(sum(subset$GENE==DEG)>0){
        celltype_group = append(celltype_group, 
                                concatenate(gsub(paste0(celltype,"_"),"",subset[,MODULE][subset$GENE==DEG]), 
                                            mysep = ", "))
        logFCs = append(logFCs, concatenate(gsub(paste0(celltype,"_"),"",subset$avg_log2FC[subset$GENE==DEG]), 
                                            mysep = ", "))
        nCelltypes = append(nCelltypes, sum(subset$GENE==DEG))
      }
    }
...
ClusterAnalysis <- function(seuratObject, 
  ...
  for(clust in show_clusters){
    ...
    # } # this is used for general subclustering
    seuratObject@misc$markers <- markers # only for subclustering 
  }
  # save before feature plots because of FeaturePlot reduction bug
  if(save_seurat) {
    saveRDS(seuratObject, paste0(saves_dir, name, "_SeuratObject.rds"))}
...
markerGeneOverlap <- function(query_markers, 
  ...
  ref_markers <- ref_markers[,-c(3,4)] # specific to Mac_subclustering
